...
class Phase103SingleHumanoidMotionAuthorityTests(unittest.TestCase):
    ...
    def test_phase1032_uses_audited_ballet_rig_articulation(self) -> None:
        ...
        self.assertIn('outward * upper_length * 0.52', port.group(0))
        self.assertIn('- Vector3.UP * upper_length * 0.30', port.group(0))
        self.assertIn('outward * reach * 0.10', port.group(0))
        self.assertIn('- Vector3.UP * reach * 0.36', port.group(0))
        self.assertIn('var hand_floor_ratio := 0.40 if opening_variant else 0.30', port.group(0))
        self.assertIn('-outward * 0.92', port.group(0))
        self.assertIn('audience_forward * 0.18', port.group(0))
