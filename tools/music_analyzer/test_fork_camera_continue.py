...
class ForkCameraContinueTests(unittest.TestCase):
    ...
    def test_production_camera_caches_metadata_and_only_interpolates_per_frame(self) -> None:
        ...
        self.assertIn("CameraState.APPROACHING_FORK", process)
        self.assertIn("CameraState.FORK_ACTIVE", process)
        self.assertIn("CameraState.LEAVING_FORK", process)
        self.assertIn("func _capture_transition(", self.camera)
        self.assertIn("var eased_progress := _smoothstep(linear_progress)", self.camera)
        self.assertIn("lerpf(_start_size, _target_size, eased_progress)", self.camera)
        self.assertIn("lerpf(_start_look_ahead, _target_look_ahead, eased_progress)", self.camera)
        self.assertIn("lerpf(_start_rig_y, _target_rig_y, eased_progress)", self.camera)
        self.assertIn("const ENTRY_DURATION := 1.05", self.camera)
        self.assertIn("const EXIT_DURATION := 1.15", self.camera)
    ...
    def test_camera_transitions_capture_once_hold_and_finish_exactly(self) -> None:
        entry = re.search(r"func _begin_fork_entry\(.*?(?=\n\nfunc )", self.camera, re.DOTALL).group(0)
        active = re.search(r"func _apply_exact_targets\(.*?(?=\n\nfunc )", self.camera, re.DOTALL).group(0)
        finish = re.search(r"func _finish_fork_exit\(.*?(?=\n\nfunc )", self.camera, re.DOTALL).group(0)
        self.assertEqual(entry.count("_capture_transition("), 1)
        self.assertIn("camera.size = _target_size", active)
        self.assertIn("camera.size = NORMAL_CAMERA_SIZE", finish)
        self.assertIn('camera_rig.set("look_ahead", NORMAL_LOOK_AHEAD)', finish)
        self.assertIn("_next_fork_index += 1", finish)
        self.assertNotIn("_next_fork_index -=", self.camera)
    ...
    def test_death_continue_restart_exit_and_policy_are_explicit(self) -> None:
        ...
        self.assertIn("_state = RunState.DEAD", self.recovery)
