"""Render a measured second-position elbow line on the final Rig for review."""
...
def main():
    ...
    candidate = solve_second_elbow_line(original, calibration["anatomical_frame"])
    ...
    for side in ("left", "right"):
        ...
        require(lines[side]["new_shoulder_minus_elbow_up"] > 0 and
                ...
                lines[side]["new_elbow_anterior_to_shoulder"] > 0,
                f"{side}: descending, anterior second-position line not achieved")
    output.mkdir(parents=True, exist_ok=True)
    previews = render_views(armature, calibration, output, prefix="second_descending")
    blend = output / "second_elbow_line_v0_6.blend"
    bpy.ops.wm.save_as_mainfile(filepath=str(blend))
    report_path = output / "report.json"
    report_path.write_text(json.dumps({
        "status": "SECOND_ELBOW_CANDIDATE_VISUAL_REVIEW_REQUIRED",
        "source_profile_sha256": reference["source_profile_sha256"],
        "source_glb_sha256": reference["source_glb_sha256"],
        "arm_lines": lines, "hand_mesh_projection": projection,
        "residuals": residuals, "hand_tip_residuals": hands,
        "previews": previews, "blend": str(blend),
        "limits": "Static second-position candidate only; changing elbow height may affect visible contour. "
                  "No temporal solve, measured joint-limit gate or ballet-teacher approval."
    }, indent=2) + "\n", encoding="utf-8")
    print("MOTION_STUDIO_SECOND_ELBOW=CANDIDATE_VISUAL_REVIEW_REQUIRED")
