...
$motionContract = Join-Path $Repo "assets\ballet_motion\lower_body_releve_motion_contract_v1.json"
$script = Join-Path $Repo "tools\blender\build_lower_body_plie_to_releve_v1.py"
$test = Join-Path $Repo "tools\ballet_motion\test_phase10_8_2_releve_motion.py"
$report = Join-Path $Repo "build\phase10_8\lower_body_motion_plie_to_releve_v1_report.json"
$previewDir = Join-Path $Repo "build\phase10_8\lower_body_motion_plie_to_releve_preview_v1"

foreach ($required in @(
    $canonical,$constraints,$grammar,$retarget,$axis,$intents,
    $staticContract,$visualContract,$motionContract,$script,$test
)) {
    if (-not (Test-Path $required)) {
        throw "Phase 10.8.2 prerequisite missing: $required"
    }
}

Write-Host "PHASE 10.8.2 - PLIE -> RELEVE FOUNDATION MOTION"
Write-Host "Endpoint authority: accepted Phase 10.6 realized poses"
Write-Host "Motion: accepted endpoint shortest-arc + synchronous minimum jerk"
Write-Host "Contact: exact plie full-foot, then per-frame forefoot root solve"
Write-Host "Intermediate plantar/toe search: NO"
...
if (-not $UseExistingArtifacts) {
    ...
    Write-Host "Reusing existing Phase 10.8.2 report and previews; Blender/test execution skipped."
}

if (-not (Test-Path $report)) {
    throw "Phase 10.8.2 report was not generated: $report"
}

$data = Get-Content $report -Raw | ConvertFrom-Json
if ($data.phase -ne "10.8.2") {
    throw "Phase 10.8.2 report phase mismatch."
}
...
foreach ($gate in @(
    ...
    "intermediate_forefoot_contact",
    "end_forefoot_contact",
    "root_rise_monotone",
    "heel_lift_monotone",
    "meaningful_final_heel_lift",
    "root_horizontal_drift_blocked",
    "nonparticipating_bones_stable",
    "semantic_preferred_envelope_proxy",
    "quaternion_flip_free",
    "intermediate_plantar_toe_search_absent"
)) {
    if (-not $data.automated_gate.$gate) {
        throw "Phase 10.8.2 automated gate failed: $gate"
    }
}

if ($data.automated_gate.glb_exported) {
    throw "GLB export is forbidden in Phase 10.8.2."
}
...
Write-Host ""
Write-Host "PHASE 10.8.2 AUTOMATED PROOF PASS"
Write-Host "Frames:         $($data.diagnostics.sample_count)"
Write-Host "Previews:       $previewCount/3"
Write-Host "Contact max:    $($data.diagnostics.maximum_contact_error)"
