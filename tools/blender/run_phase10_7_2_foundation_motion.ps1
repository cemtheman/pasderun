...
$motionContract = Join-Path $Repo "assets\ballet_motion\foundation_motion_en_avant_to_second_v1.json"
$script = Join-Path $Repo "tools\blender\build_foundation_transition_en_avant_to_second_v1.py"
$test = Join-Path $Repo "tools\ballet_motion\test_phase10_7_2_foundation_motion.py"
$report = Join-Path $Repo "build\phase10_7\foundation_motion_en_avant_to_second_v1_report.json"
$previewDir = Join-Path $Repo "build\phase10_7\foundation_motion_en_avant_to_second_preview_v1"
...
foreach ($required in @(
    ...
    if (-not (Test-Path $required)) {
        throw "Phase 10.7.2 prerequisite missing: $required"
    }
}

Write-Host "PHASE 10.7.2 - EN AVANT -> SECOND FOUNDATION MOTION"
...
if (-not $UseExistingArtifacts) {
    ...
    Write-Host "Reusing existing Phase 10.7.2 report and previews; Blender/test execution skipped."
}

if (-not (Test-Path $report)) {
    throw "Phase 10.7.2 report was not generated: $report"
}

$data = Get-Content $report -Raw | ConvertFrom-Json
if ($data.phase -ne "10.7.2") {
    throw "Phase 10.7.2 report phase mismatch."
}
...
if ($data.automated_gate.glb_exported) {
    throw "GLB export is forbidden in Phase 10.7.2."
}
...
Write-Host ""
Write-Host "PHASE 10.7.2 AUTOMATED PROOF PASS"
