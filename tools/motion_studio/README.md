...
That run separated the hand silhouettes at all 49 samples (worst +0.00230378 at the unchanged Bra Bas endpoint), but the intermediate opening still showed an angular, downward forearm at frame 37. The next candidate changes the wrist **path**, not just the opening speed: during the first-to-second leg it adds a smooth up-and-forward arc with peak offsets of 0.19 and 0.26 measured arm reaches, respectively. Both offsets vanish at the exact reviewed endpoints. It continues to re-solve the measured two-link lengths and require inward signed flexion at each sample. A report additionally records forearm vertical drop divided by arm reach on every sample; frames 34–40 fail a provisional geometric review gate if that drop exceeds 0.1. This gate does not establish ballet form or hinge anatomy. Use the same Blender command with a new output directory `build/motion_studio/guide_port_de_bras_opening_arc_v0_6`, then inspect frames 31/34/37/40/43 and the new report before adopting any motion.

The user ran this opening-arc variant on the actual final Rig. `reviews/guide_port_de_bras_geometry_screen_v0_6.json` pins the measured report and key front/side previews. Across 49 evaluated samples the minimum projected frontal hand gap is +0.00230378 armature units (frame 1), the maximum normalized forearm drop in opening frames 34–40 is 0.01763701 (frame 34), and the largest keyed elbow/wrist/hand residual is 0.00000062 armature units. The front and side images at 31/34/37/40/43 visually remove the downward forearm break previously visible at frame 37. This closes the **arm-only geometry path screen** for the Bra Bas → guide first → second probe. The stills do not validate a performed ballet movement, hand-to-body collision freedom, observed duration, footwork or a runtime asset; proceed to those gates separately before using this as an approved animation.

### Guide arm catalog batch v0.7

The two-page guide also depicts an overhead fifth/crown arm position, plus third (one arm first, one second) and fourth (one arm fifth, one second) combinations. `guide_arm_catalog.py` adds length-preserving fifth and both left/right mixed variants. `guide_arm_catalog_batch_v0_7.py` keeps the screened Bra Bas→first→second samples, adds seven connecting edges in total and keys both directions as fourteen separate Blender Actions; eight static pose Actions are included. The report lists connected routes for all 56 ordered pairs of distinct positions. The fifth wrist elevation uses the **calibrated head bone tail as a proxy** and searches a small bounded lateral range for non-overlapping frontal hand silhouettes. Frame-by-frame keyed joint residuals and hand projection are measured; front and side views of each pose and each forward edge are rendered. These are generated candidates and a diagnostic sampling grid, not timed choreography or physically verified turns.

Run on the final-Rig Windows checkout, with the prior local reports available:

```powershell
& 'C:\Program Files\Blender Foundation\Blender 5.2\blender.exe' --background --python .\tools\blender\motion_studio\guide_arm_catalog_batch_v0_7.py -- --repo . --calibration .\build\motion_studio\low_poly_girl_calibration_v0_1.json --reference .\build\motion_studio\accepted_arm_reference_v0_6.json --source-profile .\build\phase10_6\low_poly_girl_canonical_pose_solver_v1.json --clearance-report .\build\motion_studio\hand_clearance_candidate_v0_6\report.json --en-avant-report .\build\motion_studio\en_avant_outward_elbow_v0_6\report.json --first-report .\build\motion_studio\guide_first_spine_mid_v0_6\report.json --second-report .\build\motion_studio\second_inward_line_v0_6\report.json --output .\build\motion_studio\guide_arm_catalog_v0_7
```
...
The first real catalog batch was generated. `reviews/guide_catalog_bundle_screen_v0_7.json` pins the ZIP and both report digests. The fifth-crown candidate placed both hands across the face and produced a negative frontal hand gap at its endpoint and during first→fifth. The arm catalog script now searches higher crown wrists and wider lateral separation, measuring the **evaluated skinned hands for the entire first→fifth path** before selecting a trial. This is new code, not yet rerun on the final Rig. Run the arm command above with a fresh `--output .\build\motion_studio\guide_arm_catalog_overhead_v0_7` and inspect the new status and front/side crown previews. The head bone tail is only a height proxy; verify actual hair/head clearance in the rendered images.
...
The first real foot batch **did** run after the thigh/shin mapping correction: eight poses and fourteen actions were generated with a maximum sampled joint residual of 0.00000151. Its smallest tracked sole height relative to rest is −0.0063457 armature units. The report status `CANDIDATE_GEOMETRY_RENDERED` only indicates the script's loose penetration threshold was not exceeded; it does not prove two-foot contact or no sliding. First, second, fifth-left and the first→fifth-left midpoint front/side previews were screened; top views and a measured ground plane are still needed to judge turnout and floor behavior.

The overhead arm rerun is captured in `reviews/guide_arm_catalog_overhead_screen_v0_7.json`. All eight poses and fourteen arm clips rendered on the real Rig, with `front_overlap_names: []`. First→crown minimum skinned-hand gap is +0.01227945 and the catalog minimum is +0.00230336. Rendered hands no longer cover the eyes, though the crown hands approach the top sides of the hair; head/hair collision is not measured.

### Coupled body candidate v0.8

`guide_full_body_catalog_batch_v0_8.py` keys arm and foot joint targets together on the same Rig. It generates nine paired poses (Bra Bas with first feet, first, second, left/right third, left/right fourth and left/right fifth) and eight reversible connecting paths. Fourth arm paths pass through third and are synchronized to 49 foot samples. The others use 25 samples. It verifies all keyed arm and leg joint centers, skinned-hand projection, and rest-sole height at each playback frame, then renders front and side pose and midpoint PNGs. This is code awaiting its first final-Rig run; the diagnostic frames do not define real ballet tempo.

With both v0.7 reports available locally, run in the Windows checkout:

```powershell
& 'C:\Program Files\Blender Foundation\Blender 5.2\blender.exe' --background --python .\tools\blender\motion_studio\guide_full_body_catalog_batch_v0_8.py -- --repo . --calibration .\build\motion_studio\low_poly_girl_calibration_v0_1.json --reference .\build\motion_studio\accepted_arm_reference_v0_6.json --source-profile .\build\phase10_6\low_poly_girl_canonical_pose_solver_v1.json --clearance-report .\build\motion_studio\hand_clearance_candidate_v0_6\report.json --en-avant-report .\build\motion_studio\en_avant_outward_elbow_v0_6\report.json --first-report .\build\motion_studio\guide_first_spine_mid_v0_6\report.json --second-report .\build\motion_studio\second_inward_line_v0_6\report.json --arm-report .\build\motion_studio\guide_arm_catalog_overhead_v0_7\report.json --foot-report .\build\motion_studio\guide_foot_catalog_v0_7\report.json --output .\build\motion_studio\guide_full_body_catalog_v0_8
```
