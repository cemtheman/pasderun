"""Phase 10.7.2 deterministic foundation-motion timing helpers.
...
def validate_contract(contract: dict) -> None:
    if contract.get("phase") != "10.7.2":
        raise ValueError("Foundation motion contract phase mismatch.")
    transition = contract["transition"]
    if transition["start_pose"] != "en_avant":
        raise ValueError("10.7.2 must start at en_avant.")
    if transition["end_pose"] != "second":
        raise ValueError("10.7.2 must end at second.")
    duration = float(transition["duration_seconds"])
    if not 1.5 <= duration <= 2.5:
        raise ValueError("10.7.2 duration must stay in the 1.5-2.5 s proof window.")
    if int(transition["fps"]) <= 0:
        raise ValueError("FPS must be positive.")
    if contract["curve"]["type"] != "MINIMUM_JERK":
        raise ValueError("10.7.2 requires deterministic minimum-jerk timing.")
